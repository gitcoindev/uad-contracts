...
task("simulateStakingMultiplierFix", "simulate migration of one address")
  .addOptionalParam("address", "The bond holder address")
  .addOptionalParam("id", "The bond id")
  .setAction(
    async (taskArgs: { address: string; id: number }, { ethers, network }) => {
      const { address: paramAddress, id: paramId } = taskArgs;
